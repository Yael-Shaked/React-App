  ...
  return (
    ...
    <div className="bg-white rounded shadow p-6 m-4 w-full lg:w-3/4 lg:max-w-lg">
        ...
        {todos.map((todo) => (
          <li key={todo.id}>
            <div className="flex mb-4 items-center">
              ...
              <button className="flex-no-shrink p-2 border-2 rounded text-teal border-teal hover:text-white hover:bg-teal" onClick={() => { setIsPopupOpen(true); setSelectedTodo(todo); }}>Edit</button>
              </div>
          </li>
        ))}
      </ul>
      {isPopupOpen && selectedTodo && (
          <EditPopup
            onClose={() => { setIsPopupOpen(false); setSelectedTodo(null); }}
            onSave={() => handleEditTodo(selectedTodo)}
          >
            <EditTodoForm
              todo={selectedTodo}
              onEdit={handleEditTodo}
              onCancel={() => { setIsPopupOpen(false); setSelectedTodo(null); }}
            />
          </EditPopup>
      )}
    </div>
